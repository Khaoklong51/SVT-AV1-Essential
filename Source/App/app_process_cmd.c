        ...
        if (stream_status == EB_ErrorMax) {
            ...
            case 3:
                if ((int)app_cfg->frames_to_be_encoded == -1) {
                    fprintf(stderr,
                            "\rEncoding: %4d Frames @ %.2f fp%c | %.2f kbps | Size: %.2f MB | Time: %d:%02d:%02d",
                            *frame_count,
                            fps >= 1.0 ? fps : fps * 60,
                            fps >= 1.0 ? 's' : 'm',
                            ((double)(app_cfg->performance_context.byte_count << 3) * frame_rate / (app_cfg->frames_encoded * 1000)),
                            size, ete_hh, ete_mm, ete_ss);
                } else {
                    fprintf(stderr,
                            "\rEncoding: %4d/%4ld Frames @ %.2f fp%c | %.2f kbps | Size: %.2f MB [%.2f MB] | Time: %d:%02d:%02d [-%d:%02d:%02d]",
                            *frame_count,
                            (long)app_cfg->frames_to_be_encoded,
                            fps >= 1.0 ? fps : fps * 60,
                            fps >= 1.0 ? 's' : 'm',
                            ((double)(app_cfg->performance_context.byte_count << 3) * frame_rate / (app_cfg->frames_encoded * 1000)),
                            size, estsz, ete_hh, ete_mm, ete_ss, eta_hh, eta_mm, eta_ss);
                }
                break;
            default: break;
            }
